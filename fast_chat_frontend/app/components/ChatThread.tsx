...
const ChatThread: React.FC<UserCardProps> = ({
  ...
  return (
    <div className={`flex chat-card mt-2 bg-${selectedChat==position?'purple':'gray'}`} onClick={setSelection}>
      <ImageBox imageURL={"https://as2.ftcdn.net/v2/jpg/02/29/75/83/1000_F_229758328_7x8jwCwjtBMmC6rgFzLFhZoEpLobB6L8.jpg"} />
      <div className="flex-col pl-5 pt-3 w-[60%]">
        <div className="flex items-center justify-between w-full">
          <p className="name-flex text-lg font-semibold">{name}</p>
          <p>{time}</p>
        </div>
        <div className="flex items-center justify-between w-full">
